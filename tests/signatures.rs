...
mod ed25519 {
    ...
    pub fn verify_with_public_key_threadpool() {
        let key_pair = get_ed25519_key_pair_threadpool();
        let message_to_sign = b"Hello World Message To Sign".to_vec();
        let signature = ed25519_sign_with_key_pair_threadpool(key_pair, message_to_sign.clone());
    ...
    pub fn verify_with_key_pair_threadpool() {
        let key_pair = get_ed25519_key_pair();
        let message_to_sign = b"Hello World Message To Sign".to_vec();
        let signature = ed25519_sign_with_key_pair_threadpool(key_pair.clone(), message_to_sign.clone());
